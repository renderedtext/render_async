...
module RenderAsync
  module ViewHelper
    ...
    def render_async(path, options = {}, &placeholder)
      ...
      script_html_options = options.delete(:script_html_options)

      render 'render_async/render_async', **container_element_options(options),
                                          path: path,
                                          script_html_options: script_html_options,
    ...
    def polling_options(options)
      { interval: options[:interval], 
        start_poll_on_page_load: options[:start_poll_on_page_load],
        start_poll_event_name: options[:start_poll_event_name],
        stop_poll_event_name: options[:stop_poll_event_name] }
    end
